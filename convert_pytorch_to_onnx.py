...
def infer_shapes(nlp: Pipeline, framework: str) -> Tuple[List[str], List[str], Dict, BatchEncoding]:
    ...
    tokens = nlp.tokenizer("This is a sample output")
...
def munual_export():
    model, tokenizer = pre_load()
    input_names = ["input_ids", "attention_mask", "token_type_ids"]
    inputs = tokenizer.encode_plus("Hello World", padding='max_length', truncation=True, max_length=32, return_tensors='pt')
    # print(inputs)
    order_input_name, model_args = ensure_valid_input(model, inputs, input_names)
    print(order_input_name)
    print("+++++++++++++++")
    print(model_args)
    # input_data = torch.randn(1, 3, 32)
    output_path = "D:\\VirtualMachineImage\\Filter.onnx"
    input_names = ["input_ids", "attention_mask", "token_type_ids"]
